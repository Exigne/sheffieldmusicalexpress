...
const PASSCODE = "STEELCITY"; // <-- Change this to your secret password
...
export default async function AdminDashboard() {
  ...
  async function addGig(formData: FormData) {
    ...
    await sql`INSERT INTO gigs (title, venue, gig_date, price, description) VALUES (${formData.get('title') as string}, ${formData.get('venue') as string}, ${formData.get('gig_date') as string}, ${formData.get('price') as string}, ${formData.get('description') as string})`;
        ...
        <div className="form-card">
          ...
          <div style={{ padding: '20px' }}>
            <form action={addGig} style={{ display: 'flex', flexDirection: 'column', gap: '15px' }}>
              <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: '15px' }}>
                <input name="title" className="form-input" placeholder="Event Title" required />
                ...
                <input name="price" className="form-input" placeholder="Price" required />
              </div>
              <textarea name="description" className="reply-textarea" rows={2} placeholder="Optional info..." />
              <div style={{ display: 'flex', gap: '10px' }}>
                <input name="passcode" type="password" className="form-input" placeholder="Admin Passcode" style={{ width: '200px' }} required />
