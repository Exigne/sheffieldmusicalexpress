...
export default function AlbumReviewsModal() {
  ...
  return (
    <Modal>
      ...
      <div style={{ background: 'white', border: '1px solid #ddd', padding: '30px', display: 'flex', alignItems: 'center', gap: '25px', marginBottom: '30px' }}>
        ...
        <div>
          ...
          <p style={{ margin: '5px 0 0 0', color: '#666', fontSize: '1rem' }}>
            Steel City's verdict on the latest releases, local demos, and classics.
          </p>
        </div>
      </div>
      ...
      {/* 3. THREAD LISTING */}
      {loading ? (
        <p style={{ fontFamily: 'IBM Plex Mono', color: '#666' }}>Reading the liner notes...</p>
      ) : threads.length === 0 ? (
        <p style={{ padding: '20px 0', color: '#666' }}>No reviews posted yet. Be the first!</p>
      ) : (
      ...
      <div style={{ marginTop: '20px', textAlign: 'center' }}>
        <Link href="/boards/albums" style={{ color: 'var(--rust)', fontWeight: 'bold', textDecoration: 'none', fontSize: '0.9rem' }}>
          VIEW FULL BOARD →
        </Link>
      </div>
    </Modal>
